...
// TODO： Add black_box
// TODO: Move non-benchmark code to src directory.

/// Generates inputs including two point vectors.
pub fn generate_elliptic_inputs<G>(size: usize) -> (Vec<G>, Vec<G>)
where
    G: ProjectiveCurve,
{
    let mut rng = test_rng();
    (
        (0..size).map(|_| G::rand(&mut rng)).collect::<Vec<_>>(),
        (0..size).map(|_| G::rand(&mut rng)).collect::<Vec<_>>(),
    )
}
...
pub fn add<G>(point_vec1: &Vec<G>, point_vec2: &Vec<G>) -> Vec<G>
where
    G: ProjectiveCurve,
{
    point_vec1
...
pub fn projective_scalar_mul_assign<P>(points: &mut Vec<P>, scalars: &Vec<P::ScalarField>)
//->  Vec<P>
where
    P: ProjectiveCurve,
{
...
fn elliptic_curve_add(c: &mut Criterion) {
    ...
    for size in (8..20).step_by(2) {
        ...
        group.bench_function(
            ...
            |b| {
                b.iter(|| {
                    let _ = add::<G1Projective>(&lhs, &rhs);
                })
            },
        );
    }
